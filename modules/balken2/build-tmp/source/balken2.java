...
public class balken2 extends PApplet {
...
float zeigerMin;
float minn;
int minFarbe = color(48, 56, 58); //backgroundfarbe
...
float maxx;
int maxFarbe = color(255);
//color maxFarbe = color(57, 122, 156);

//treshold
float treshold = 300;
int tresholdColor = color(123, 50, 34);
...
//skalen nummern
boolean scale = true;
int skalenFarbe = color(150,150,150);

//farben generell
int backGround = color(48, 55, 57); //hintergrundfarbe
int barGraph = color(56,135,166);
//color barGraph = color(144, 197, 194);  //aktueller wert als bargraph
int barGraphGround = color(152, 156, 156);
...
//zufallsgenerator
float generator;
float startWert = 0.2f;
float incrementWert = 0.005f;
...
public void timeLapsFill(float sensorValue){
...
public float timelapsValue(float[] timelapsArr, float sliderValue) {
  int ArrPosition = PApplet.parseInt(map(sliderValue, 0, 150, 0 , timelapsArr.length-1));
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "balken2" };
    if (passedArgs != null) {
